...
void ImportDialog::on_importBtn_clicked()
{
    ...
    if( ui->privateKeyLineEdit->text().endsWith(".key") )
    {
        ...
        if( file.open(QIODevice::ReadOnly))
        {
            ...
            GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key", "wallet_import_private_key", QStringList() << str));
            ui->importBtn->setEnabled(false);
            shadowWidget->show();
        ...
        if(importEnterPwdDialog.pop())
        {
            ...
            if( file.open(QIODevice::ReadOnly))
            {
                ...
                if( output.startsWith("privateKey="))
                {
                    ...
                    GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key", "wallet_import_private_key", QStringList() << pk));
                    ui->importBtn->setEnabled(false);
                    shadowWidget->show();
                }
                else
    ...
    else // 如果输入框中是 私钥
    {
        ...
        {
            GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key", "wallet_import_private_key", QStringList() << privateKey));
...
void ImportDialog::jsonDataUpdated(QString id)
{
    if( id == "id_wallet_import_private_key")
    {
        ...
        if( result.mid(0,9) == "\"result\":")
        {
            ...
            GDW::getInstance()->needToScan = true;  // 导入成功，需要scan
            ...
            else
            {
                ...
                int size = GDW::getInstance()->addressMap.size();
...
//                rpcThread->start();
                ...
                GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_scan_transaction", "wallet_scan_transaction", QStringList() << "ALL" << "true" ));
...
//                emit accepted();
        ...
        else if( result.mid(0,8) == "\"error\":")
        {
            ...
            {
                GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key_" + name, "wallet_import_private_key", QStringList() << privateKey << name << "true"));
                shadowWidget->show();
                return;
            }
            else // 如果输入框中是 私钥
            {
                GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_import_private_key_" + name, "wallet_import_private_key", QStringList() << ui->privateKeyLineEdit->text() << name << "true" ));
                shadowWidget->show();
            }
        }
...
bool ImportDialog::accountNameAlreadyExisted(QString name)
{
    ...
    foreach (str, keys)
    {
        if( GDW::getInstance()->configFile->value(str) == name)
        {
            existed = true;
            break;
        }
