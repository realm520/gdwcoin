    ...
    ui(new Ui::SetDialog)
{
    ...
    ui->lockTimeSpinBox->setValue(QString::number(GDW::getInstance()->lockMinutes).toInt());
    ...
    ui->minimizeCheckBox->setChecked( GDW::getInstance()->minimizeToTray);
    ui->closeCheckBox->setChecked( GDW::getInstance()->closeToMinimize);
    ui->resyncCheckBox->setChecked( GDW::getInstance()->resyncNextTime);
    ...
    QRegExp rx("^(10000|[1-9]\\d{0,3})(?:\\.\\d{0,5})?$|(^\\t?$)");
...
void SetDialog::on_saveBtn_clicked()
{
    ...
    {
        GDW::getInstance()->notProduce = false;
    }
    else
    {
        GDW::getInstance()->notProduce = true;
    }
    GDW::getInstance()->configFile->setValue("/settings/notAutoLock", !GDW::getInstance()->notProduce);
    ...
    {
        GDW::getInstance()->configFile->setValue("/settings/language", "English");
        GDW::getInstance()->language = "English";
    }


    GDW::getInstance()->minimizeToTray = ui->minimizeCheckBox->isChecked();
    GDW::getInstance()->configFile->setValue("/settings/minimizeToTray", GDW::getInstance()->minimizeToTray);

    GDW::getInstance()->closeToMinimize = ui->closeCheckBox->isChecked();
    GDW::getInstance()->configFile->setValue("/settings/closeToMinimize", GDW::getInstance()->closeToMinimize);

    GDW::getInstance()->resyncNextTime = ui->resyncCheckBox->isChecked();
    GDW::getInstance()->configFile->setValue("/settings/resyncNextTime", GDW::getInstance()->resyncNextTime);
...
void SetDialog::on_confirmBtn_clicked()
{
    ...
    GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_check_passphrase", "wallet_check_passphrase", QStringList() << ui->oldPwdLineEdit->text() ));
...
void SetDialog::jsonDataUpdated(QString id)
{
    if( id == "id_wallet_check_passphrase")
    {
        ...
        {
            GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_change_passphrase", "wallet_change_passphrase", QStringList() << ui->oldPwdLineEdit->text() <<  ui->newPwdLineEdit->text() ));
        }
        else
        {
