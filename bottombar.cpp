...
//    DLOG_QT_WALLET_FUNCTION_BEGIN;
    ...
    QString result = GDW::getInstance()->jsonDataValue("id_info");
...
//        int numToSync = seconds.toInt()/ 10;
        ...
        GDW::getInstance()->currentBlockHeight = num.toInt();
        ...
        if( GDW::getInstance()->needToScan && scanPercent > 0.99999)
        {
            GDW::getInstance()->postRPC( toJsonFormat( "id_scan", "scan", QStringList() << "0"));
            GDW::getInstance()->needToScan = false;
        }
