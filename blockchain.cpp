...
TwoAddresses GDW::getAddress(QString name)
{
...
bool GDW::isMyAddress(QString address)
{
    if( address.isEmpty())
    {
        return false;
...
QString GDW::addressToName(QString address)
{
    foreach (QString key, addressMap.keys())
    {
...
QString GDW::getBalance(QString name)
{
    if( name.isEmpty())
    {
        return NULL;
...
void GDW::getSystemEnvironmentPath()
{
    ...
    foreach(str,environment)
    {
        ...
        {
            walletConfigPath = str.mid(5) + "/Library/Application Support/GDWWallet";
            appDataPath = walletConfigPath + "/chaindata";
            qDebug() << "appDataPath:" << appDataPath;
            break;
        }
    ...
    foreach(str,environment)
    {
        if (str.startsWith("HOME="))
        {
            ...
            walletConfigPath = str.mid(5) + "/GDWWallet";
            qDebug() << "appDataPath:" << appDataPath;
            break;
        }
    }
...
void GDW::changeToWalletConfigPath()
{
...
int GDW::getAssetId(QString symbol)
{
    int id = -1;
    foreach (int key, GDW::getInstance()->assetInfoMap.keys())
    {
        AssetInfo info = GDW::getInstance()->assetInfoMap.value(key);
...
void GDW::parseTransactions(QString result, QString accountName)
{
...
void GDW::updateAllContractBalance()
{
    ...
    foreach (QString key, keys)
    {
        ...
        {
            updateAccountContractBalance(GDW::getInstance()->addressMap.value(key).ownerAddress,contract);
        }
    }
}
...
void GDW::collectContractTransactions(QString contractAddress)
{
    if( ERC20TokenInfoMap.keys().contains(contractAddress))
    {
        ...
        if(toBlockHeight == ERC20TokenInfoMap.value(contractAddress).lastQueryBlockHeight)   return;
        postRPC( toJsonFormat( "id_blockchain_list_contract_transaction_history+" + contractAddress + "+" + QString::number(toBlockHeight),
                               ...
                               << contractAddress << "14"));
        ERC20TokenInfoMap[contractAddress].lastQueryBlockHeight = toBlockHeight;

        qDebug() << "jjjjjjjjj " << toJsonFormat( "id_blockchain_list_contract_transaction_history+" + contractAddress + "+" + QString::number(toBlockHeight),
                                                  "blockchain_list_contract_transaction_history",
                                                  QStringList() << QString::number(fromBlockHeight) << QString::number(toBlockHeight)
                                                  << contractAddress << "14");
    }
...
QString GDW::jsonDataValue(QString id)
{
...
double GDW::getPendingAmount(QString name)
{
    mutexForConfigFile.lock();
...
QString GDW::getPendingInfo(QString id)
{
    mutexForConfigFile.lock();
    if( !pendingFile->open(QIODevice::ReadOnly))
    {
...
int GDW::getDelegateIndex(QString delegateName)
{
    mutexForDelegateList.lock();
    QStringList singleList = GDW::getInstance()->delegateList.filter( "\"name\":\"" + delegateName + "\"");
...
QString GDW::registerMapValue(QString key)
{
...
void GDW::registerMapInsert(QString key, QString value)
{
    mutexForRegisterMap.lock();
...
int GDW::registerMapRemove(QString key)
{
    mutexForRegisterMap.lock();
...
QString GDW::balanceMapValue(QString key)
{
...
void GDW::balanceMapInsert(QString key, QString value)
{
    mutexForBalanceMap.lock();
...
int GDW::balanceMapRemove(QString key)
{
    mutexForBalanceMap.lock();
...
TwoAddresses GDW::addressMapValue(QString key)
{
...
void GDW::addressMapInsert(QString key, TwoAddresses value)
{
    mutexForAddressMap.lock();
...
int GDW::addressMapRemove(QString key)
{
    mutexForAddressMap.lock();
...
bool GDW::rpcReceivedOrNotMapValue(QString key)
{
...
void GDW::rpcReceivedOrNotMapSetValue(QString key, bool received)
{
    mutexForRpcReceiveOrNot.lock();
...
void GDW::appendCurrentDialogVector( QWidget * w)
{
    currentDialogVector.append(w);
}

void GDW::removeCurrentDialogVector( QWidget * w)
{
    currentDialogVector.removeAll(w);
}

void GDW::hideCurrentDialog()
{
...
void GDW::resetPosOfCurrentDialog()
{
    foreach (QWidget* w, currentDialogVector)
    {
...
void GDW::initWorkerThreadManager()
{
...
void GDW::destroyWorkerThreadManager()
{
...
{
    mutexForAddressMap.lock();
    for (QMap<QString,TwoAddresses>::const_iterator i = GDW::getInstance()->addressMap.constBegin(); i != GDW::getInstance()->addressMap.constEnd(); ++i)
    {
        if(i.key() == strName)
        {
            mutexForAddressMap.unlock();
