    ...
    ui(new Ui::AssetIssueDialog)
{
    ...
    AssetInfo info = GDW::getInstance()->assetInfoMap.value(id);
...
void AssetIssueDialog::on_okBtn_clicked()
{
    ...
    GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_uia_issue", "wallet_uia_issue", QStringList() << ui->amountLineEdit->text()
...
void AssetIssueDialog::jsonDataUpdated(QString id)
{
    if( id == "id_wallet_uia_issue")
    {
        ...
        {
            GDW::getInstance()->scanLater();

            ui->amountLineEdit->clear();

            int pos = result.indexOf("\"entry_id\":") + 12;
            QString txId = result.mid( pos, result.indexOf( "\"", pos ) - pos );
            GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_scan_transaction", "wallet_scan_transaction", QStringList() << txId << "true" ));
