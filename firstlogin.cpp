...
//        rpcThread->start();
        ...
        GDW::getInstance()->postRPC( toJsonFormat( "id_create", "create", QStringList() << "wallet" << ui->pwdLineEdit->text() ));
    }
    else
    {
...
void FirstLogin::jsonDataUpdated(QString id)
{
    if( id == "id_create")
    {
        ...
        {
            GDW::getInstance()->postRPC( toJsonFormat( "id_unlock_firstlogin", "unlock", QStringList() << "99999999" << ui->pwdLineEdit->text() ));
qDebug() << "id_unlock_firstlogin";
    ...
    if( id == "id_unlock_firstlogin")
    {
        ...
        qDebug() << "id_unlock_firstlogin" << GDW::getInstance()->jsonDataValue(id);
        if( GDW::getInstance()->jsonDataValue(id) == "\"result\":null")
        {
            emit login();
            this->close();
        }
