...
//                  );
    ...
    if( !GDW::getInstance()->configFile->contains("/settings/chainPath") )
    {
    ...
    {
        QString path = GDW::getInstance()->configFile->value("/settings/chainPath").toString();
        QDir dir(path);
        ...
        if( !dir.exists() || ( !dir2.exists() && !dir3.exists())  )  // 如果数据文件被删除了
        {
            ...
            GDW::getInstance()->configFile->clear();
            GDW::getInstance()->configFile->setValue("/settings/lockMinutes",5);
            GDW::getInstance()->lockMinutes     = 5;
            GDW::getInstance()->configFile->setValue("/settings/notAutoLock",false);
            GDW::getInstance()->notProduce      =  true;
            GDW::getInstance()->configFile->setValue("/settings/language","Simplified Chinese");
            GDW::getInstance()->language = "Simplified Chinese";
            GDW::getInstance()->minimizeToTray  = false;
        ...
        else
        {
            ...
            GDW::getInstance()->appDataPath = GDW::getInstance()->configFile->value("/settings/chainPath").toString();
            ...
            if( GDW::getInstance()->configFile->value("/settings/resyncNextTime",false).toBool())
            {
                ...
                QFile file(GDW::getInstance()->configFile->value("/settings/chainPath").toString() + "\\config.json");
            ...
            if( GDW::getInstance()->proc->waitForStarted())
            {
                qDebug() << "laungh gdw succeeded";
            }
            else
            {
                qDebug() << "laungh gdw failed";
            }
#endif
...
//    QTimer::singleShot(1000,mainPage,SLOT(show()));
    ...
    if(GDW::getInstance()->notProduce)
    {
        timer->start(GDW::getInstance()->lockMinutes * 60000);
    }
...
void Frame::getAccountInfo()
{
    ...
    GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_list_my_addresses", "wallet_list_my_addresses", QStringList() << ""));

    GDW::getInstance()->postRPC( toJsonFormat( "id_balance", "balance", QStringList() << ""));

    GDW::getInstance()->updateAllContractBalance();

    GDW::getInstance()->collectContracts();
...
void Frame::showLockPage()
{
    ...
    GDW::getInstance()->postRPC( toJsonFormat( "id_lock", "lock", QStringList() << "" ));
qDebug() << "lock ";
    DLOG_QT_WALLET_FUNCTION_END;
}
...
qDebug() << "unlock showCurrentDialog";
    GDW::getInstance()->showCurrentDialog();

    if( GDW::getInstance()->consoleWidget)
    {
        GDW::getInstance()->consoleWidget->showNormal();
...
void Frame::syncFinished()
{
    ...
    GDW::getInstance()->initWorkerThreadManager();

    if( GDW::getInstance()->contactsFile == NULL)
    {
        GDW::getInstance()->getContactsFile();
    }
...
void Frame::showTransferPage()
{
    closeCurrentPage();
    getAccountInfo();
    transferPage = new TransferPage(GDW::getInstance()->currentAccount,centralWidget);
    transferPage->setAttribute(Qt::WA_DeleteOnClose);
    connect(transferPage,SIGNAL(accountChanged(QString)),this,SLOT(showTransferPage(QString)));
    connect(transferPage,SIGNAL(showShadowWidget()),this,SLOT(shadowWidgetShow()));
    connect(transferPage,SIGNAL(hideShadowWidget()),this,SLOT(shadowWidgetHide()));
...
bool Frame::eventFilter(QObject* watched, QEvent* e)
{
    ...
    if( GDW::getInstance()->currentDialogVector.contains(  (QWidget*)watched) )
    {
        ...
        GDW::getInstance()->resetPosOfCurrentDialog();  // currentDialog 一起移动
...
void Frame::jsonDataUpdated(QString id)
{
    if(id == "id_open")
    {
        ...
        {
            // 其他情况视为钱包数据损坏
            QDir dir(GDW::getInstance()->appDataPath + "/wallets/wallet");
...
//            commonDialog.setText(QString::fromLocal8Bit("钱包数据损坏，请重新启动: ") + errorMessage );
    ...
    else if(id == "id_wallet_list_my_addresses")
    {
        ...
        {
            QString addName = GDW::getInstance()->configFile->value("/accountInfo/" + key).toString();
            if( !accountList.contains( addName) && !addName.isEmpty())
            {
                // 如果config记录的账户钱包中没有 则清除config文件中记录以及内存中各map
                GDW::getInstance()->deleteAccountInConfigFile( addName);
                GDW::getInstance()->addressMapRemove( addName);
                GDW::getInstance()->balanceMapRemove( addName);
                GDW::getInstance()->registerMapRemove( addName);
            }
        }
        ...
        foreach (QString key, keys)
        {
            QString addName = GDW::getInstance()->configFile->value(key).toString();
            GDW::getInstance()->balanceMapInsert(addName, GDW::getInstance()->getBalance( addName));
            GDW::getInstance()->registerMapInsert(addName, GDW::getInstance()->getRegisterTime( addName));
            GDW::getInstance()->addressMapInsert(addName, GDW::getInstance()->getAddress (addName));
        ...
        if( !accountList.isEmpty())    // 把config中没有记录的账号写入config中
        {
            for( int i = 0; i < accountList.size(); i++)
            {
                ...
                QString accountName = QString(QString::fromLocal8Bit("账户") + toThousandFigure(GDW::getInstance()->balanceMap.size() + 1));
                mutexForBalanceMap.unlock();
                GDW::getInstance()->configFile->setValue("/accountInfo/" + accountName, addName);
                GDW::getInstance()->balanceMapInsert(addName, GDW::getInstance()->getBalance( addName));
                GDW::getInstance()->registerMapInsert(addName, GDW::getInstance()->getRegisterTime( addName));
                GDW::getInstance()->addressMapInsert(addName, GDW::getInstance()->getAddress ( addName));
            }
    ...
    else if(id == "id_lock")
    {
        ...
        if( result == "\"result\":null")
        {
            ...
            GDW::getInstance()->hideCurrentDialog();
    ...
    else if(id == "id_blockchain_list_assets")
    {
        ...
        GDW::getInstance()->updateAllToken();
...
//                            GDW::getInstance()->ERC20TokenInfoMap[id]. = description;
    ...
    if( id.startsWith( "id_contract_call_offline_+") )
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            else if( func == "precision")
            {
                ...
                precision.remove('\"');
                GDW::getInstance()->ERC20TokenInfoMap[contractAddress].precision = precision.toDouble();
            }
            else if( func == "admin")
            {
    ...
    if( id.startsWith( "id_contract_call_offline_balanceOf+") )
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            if( GDW::getInstance()->accountContractBalanceMap.contains(accountAddress))
            {
                map = GDW::getInstance()->accountContractBalanceMap[accountAddress];
            }
            map.insert(contractAddress,balance);
            GDW::getInstance()->accountContractBalanceMap.insert(accountAddress,map);
        }
    ...
    if( id.startsWith( "id_blockchain_list_contract_transaction_history+") )
    {
        ...
        if( result.startsWith("\"result\":"))
        {
            ...
            GDW::getInstance()->ERC20TokenInfoMap[contractAddress].collectedBlockHeight = toBlockHeight.toInt();
            ...
            if(json_error.error == QJsonParseError::NoError)
            {
                if( parse_doucment.isObject())
                {
                    ...
                    if( jsonObject.contains("result"))
                    {
                        ...
                        if( GDW::getInstance()->isMyAddress(contractTransaction.fromAddress))
                        {
                            QString key = contractTransaction.fromAddress + "-" + contractTransaction.contractAddress;
                            GDW::getInstance()->accountContractTransactionMap[key].append(contractTransaction);
                        }

                        if( GDW::getInstance()->isMyAddress(contractTransaction.toAddress) && contractTransaction.fromAddress != contractTransaction.toAddress)
                        {
...
void Frame::scan()
{
    GDW::getInstance()->postRPC( toJsonFormat( "id_scan", "scan", QStringList() << "0"));
}
