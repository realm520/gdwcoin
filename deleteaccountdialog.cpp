...
void DeleteAccountDialog::jsonDataUpdated(QString id)
{
    if( id.startsWith( "id_wallet_check_passphrase") )
    {
        ...
        {
            GDW::getInstance()->postRPC( toJsonFormat( "id_wallet_account_delete", "wallet_account_delete", QStringList() << accountName
                                                       ));
    ...
    if( id == "id_wallet_account_delete" )
    {
        ...
        if( result == "\"result\":true")
        {
            GDW::getInstance()->deleteAccountInConfigFile( accountName);
            GDW::getInstance()->addressMapRemove( accountName);
            GDW::getInstance()->balanceMapRemove( accountName);
            GDW::getInstance()->registerMapRemove( accountName);
