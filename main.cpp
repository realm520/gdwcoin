...
bool checkOnly()
{
    ...
    //  创建互斥量
    HANDLE m_hMutex  =  CreateMutex(NULL, FALSE,  L"GDWWALLET" );
    //  检查错误代码
...
bool checkOnly()
{
    const char filename[] = "/tmp/gdwwalletlockfile";
...
//    a.installTranslator(&translator);
    GDW::getInstance();  // 在frame创建前先创建实例，读取language
...
//    qInstallMessageHandler(outputMessage);  // 重定向qebug 到log.txt
    ...
    Frame frame;
    GDW::getInstance()->mainFrame = &frame;   // 一个全局的指向主窗口的指针
    frame.show();
