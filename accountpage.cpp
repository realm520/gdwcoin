...
void AccountPage::updateBalance()
{
    if( ui->assetComboBox->currentIndex() == 0)
    {
        ...
        AssetBalanceMap assetBalanceMap = GDW::getInstance()->accountBalanceMap.value(accountName);
        int assetId = GDW::getInstance()->getAssetId(ui->assetComboBox->currentText());
        if( assetId < 0)    return;
        unsigned long long amount = assetBalanceMap.value(assetId);
        AssetInfo info = GDW::getInstance()->assetInfoMap.value(assetId);
...
void AccountPage::showNormalTransactions()
{
    ...
    TransactionsInfoVector vector = GDW::getInstance()->transactionsMap.value(accountName + "_" + ui->assetComboBox->currentText());
    ...
    for(int i = rowCount - 1; i > -1; i--)
    {
        ...
        // 对方账户
        ui->accountTransactionsTableWidget->setItem(i,3,new QTableWidgetItem( GDW::getInstance()->addressToName(detail.opposite)));
...
//            result.opposite = QString::fromLocal8Bit("调用合约");
        ...
        else if( entry.fromAccount == accountName || entry.fromAccount == GDW::getInstance()->addressMap.value(accountName).ownerAddress)
        {
        ...
        {
            if( entry.toAccount == accountName || entry.toAccount == GDW::getInstance()->addressMap.value(accountName).ownerAddress)
            {
...
{
    QRCodeDialog qrcodeDialog(GDW::getInstance()->addressMapValue(accountName).ownerAddress);
