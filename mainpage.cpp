...
void MainPage::addAccount()
{
    ...
    if( !name.isEmpty())
    {
        ...
        GDW::getInstance()->write("wallet_account_create " + name + '\n');
        QString result = GDW::getInstance()->read();
...
void MainPage::updateAccountList()
{
    ...
    if( assetIndex <= 0)
    {
        ...
        {
            QString accountName = GDW::getInstance()->configFile->value( keys.at( i)).toString();
            int rowNum = i;

            ui->accountTableWidget->setRowHeight(rowNum,57);
            ui->accountTableWidget->setItem(rowNum,0,new QTableWidgetItem(accountName));
            ui->accountTableWidget->setItem(rowNum,1,new QTableWidgetItem(GDW::getInstance()->addressMapValue(accountName).ownerAddress));
...
//    ui->totalBalanceLabel->adjustSize();
    ...
    if( assetIndex <= 0)
    {
        assetIndex = 0;
        AssetInfo info = GDW::getInstance()->assetInfoMap.value(assetIndex);
        foreach (QString key, GDW::getInstance()->accountBalanceMap.keys())
        {
    ...
    else
    {
        ...
        {
            QString accountAddress = GDW::getInstance()->addressMap.value(key).ownerAddress;
            totalBalance += GDW::getInstance()->accountContractBalanceMap.value(accountAddress).value(contractAddress);
        }
...
void MainPage::updatePending()
{
    ...
    QStringList recordKeys = GDW::getInstance()->configFile->childKeys();
    GDW::getInstance()->configFile->endGroup();
    ...
    foreach (QString ss, strList)
    {
        ...
        if( GDW::getInstance()->configFile->value("recordId/" + sList.at(0)).toInt() != 0 )
        {
            strList.removeAll( ss);
            continue;
        }
        ...
        if( !GDW::getInstance()->configFile->contains("recordId/" + sList.at(0)))
        {
            strList.removeAll( ss);
            continue;
        }
...
void MainPage::renameAccount(QString name)
{
    ...
    {
        GDW::getInstance()->write("wallet_account_rename " + name + " " + newName + '\n');
        QString result = GDW::getInstance()->read();
        qDebug() << result;
        if( result.mid(0,2) == "OK")
        {
            ...
            foreach (QString key, keys)
            {
                if( GDW::getInstance()->configFile->value(key) == name)
                {
                    GDW::getInstance()->configFile->setValue(key, newName);
                    break;
                }
            }
            GDW::getInstance()->configFile->endGroup();
            mutexForConfigFile.unlock();
            GDW::getInstance()->balanceMapInsert( newName, GDW::getInstance()->balanceMapValue(name));
            GDW::getInstance()->balanceMapRemove(name);
            GDW::getInstance()->registerMapInsert( newName, GDW::getInstance()->registerMapValue(name));
            GDW::getInstance()->registerMapRemove(name);
            GDW::getInstance()->addressMapInsert( newName, GDW::getInstance()->addressMapValue(name));
            GDW::getInstance()->addressMapRemove(name);
